...
# Added Cifar100Dataset
class Cifar100Dataset(DAGANDataset):
    def __init__(self, batch_size, last_training_class_index, reverse_channels, num_of_gpus, gen_batches):
        super(Cifar100Dataset, self).__init__(batch_size, last_training_class_index, reverse_channels, num_of_gpus,
                                              gen_batches)

    def reshape(self, data):
        return np.broadcast_to(data, (100, data.shape[0], data.shape[1], data.shape[2], data.shape[3]))

    def load_dataset(self, gan_training_index):
        (x_train, y_train), (x_test, y_test) = tf.keras.datasets.cifar100.load_data(label_mode='fine')
        # Shape of the data should be in form of [num_classes, num_samples, im_height, im_width, im_channels].
